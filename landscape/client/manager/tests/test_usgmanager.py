...
from landscape.client.manager.usgmanager import USG_EXECUTABLE, UsgManager, USG_NOT_FOUND, TAILORING_FILE_DIR
...
class UsgManagerTests(LandscapeTest):
    ...
    def setUp(self):
        ...
        self.spawn_mock = mock.patch(MODULE + ".reactor.spawnProcess", side_effect=process_success).start()
        self.audit_result_glob = mock.patch(MODULE + ".USG_AUDIT_RESULTS_GLOB", new=os.path.join(self.tempdir, "usg-results-*.xml")).start()
    ...
    def test_audit(self):
        ...
        send_message = mock.patch.object(self.plugin.registry.broker, "send_message", new=mock.AsyncMock()).start()
        audit_result = os.path.join(self.tempdir, "usg-results-TEST.xml")

        with open(audit_result, "w") as arfp:
            arfp.write("<test>TEST</test>\n")

        deferred = ensureDeferred(self.plugin.handle_usg_message({
            "operation-id": 1,
            "action": "audit",
        ...
        def check(_):
            ...
            self.spawn_mock.assert_called_once_with(mock.ANY, USG_EXECUTABLE, args=["audit", "cis_level1_workstation"])
            which.assert_called_once_with(USG_EXECUTABLE)
            self.assertEqual(send_message.mock_calls, [
                mock.call(
                    {
    ...
    def test_no_usg(self):
        """Any message results in a failure response if USG is not available."""
        which = mock.patch("shutil.which").start()
    ...
    def test_fix_reboot_required(self):
        """A successful 'usg fix' results in the reboot required flag being set."""
        which = mock.patch("shutil.which").start()
        ...
        def check(_):
            ...
            self.spawn_mock.assert_called_once_with(mock.ANY, USG_EXECUTABLE, args=["fix", "cis_level1_workstation"])
            which.assert_called_once_with(USG_EXECUTABLE)
            send_message.assert_has_calls([
                mock.call(
                    {
    ...
    def test_tailoring_file(self):
        """When a tailoring file is provided, it is downloaded and passed to USG CLI."""
        mock.patch("shutil.which").start()
        mock.patch.object(self.plugin.registry.broker, "send_message", new=mock.AsyncMock()).start()
        save_attachments = mock.patch(MODULE + ".save_attachments", new=mock.AsyncMock()).start()
        ...
        def check(_):
            tailoring_file_path = os.path.join(self.plugin.registry.config.data_path, TAILORING_FILE_DIR, "test-tailoring.xml")
            self.spawn_mock.assert_called_once_with(
                mock.ANY,
                USG_EXECUTABLE,
                args=["audit", "cis_level1_workstation", "--tailoring-file", tailoring_file_path],
            )
    ...
    def test_process_failed(self):
        """When the USG process errors, the activity fails and the error is reported."""
        which = mock.patch("shutil.which").start()
        send_message = mock.patch.object(self.plugin.registry.broker, "send_message", new=mock.AsyncMock()).start()

        def process_fail(protocol, *args, **kwargs):
            protocol.result_deferred.errback(ProcessFailedError(b"failed!", -1))
    ...
    def test_other_exception(self):
        """If any other exception is raise, the activity fails and the error is reported."""
        which = mock.patch("shutil.which").start()
        send_message = mock.patch.object(self.plugin.registry.broker, "send_message", new=mock.AsyncMock()).start()
        self.spawn_mock.side_effect = ValueError("something terrible has happened")
