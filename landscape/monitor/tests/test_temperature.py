...
def create_thermal_zone(thermal_zone_dir, zone_dir="ZONE1", temperature=50):
    """Create temperature data and return a thermal zone dir name."""
    zone_dir_name = os.path.join(thermal_zone_dir, zone_dir)
    if not os.path.isdir(zone_dir_name):
        os.makedirs(zone_dir_name)

    file = open(os.path.join(zone_dir_name, "temperature"), "w")
    file.write("\ntemperature:             %d C\n" % temperature)
    file.close()
    return zone_dir_name


class TemperatureTestWithSampleData(LandscapeTest):
    ...
    def setUp(self):
        ...
        self.thermal_zone_dir = tempfile.mkdtemp()
        create_thermal_zone(self.thermal_zone_dir, "ZONE1")

    def tearDown(self):
        """Deinitialize test helpers and remove sample thermal zone."""
        LandscapeTest.tearDown(self)
        shutil.rmtree(self.thermal_zone_dir)
    ...
    def test_wb_disabled_with_no_thermal_zones(self):
        ...
        """
        thermal_zone_dir = tempfile.mkdtemp()
        os.rmdir(thermal_zone_dir)
        plugin = Temperature(thermal_zone_dir=thermal_zone_dir)
    ...
    def test_no_messages_without_thermal_zones(self):
        ...
        """
        thermal_zone_dir = self.make_dir()
        plugin = Temperature(interval=1, thermal_zone_dir=thermal_zone_dir)
    ...
    def test_disjointed_thermal_zone_temperature_changes(self):
        ...
        """
        create_thermal_zone(self.thermal_zone_dir, "ZONE2")
        plugin = Temperature(thermal_zone_dir=self.thermal_zone_dir,
        ...
        self.update_temperature("ZONE2", 56)
        self.reactor.advance(step_size)
