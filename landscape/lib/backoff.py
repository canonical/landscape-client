...
class ExponentialBackoff:
    ...
    def get_delay(self):
        '''
        Calculates the delay using formula that gives this chart where start
        is 5 seconds and max is 60 seconds. Exponent factor is 2
                Count  Delay
                0      0
                1      5
                2      10
    ...
    def get_random_delay(self, percent_rand=0.25):
        '''
        Adds randomness to the specified percentage of the delay. For example
        for a delay of 12 and 25%, it works out to 9 + rand(0,3)
        '''
        delay = self.get_delay()
        non_random_part = delay * (1-percent_rand)
        random_part = delay * percent_rand * random.random()
