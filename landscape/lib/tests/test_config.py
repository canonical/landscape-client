...
from landscape.lib.testing import HelperTestCase, ConfigTestCase, LogKeeperHelper
...
class BaseConfigurationTest(ConfigTestCase, HelperTestCase, unittest.TestCase):
    ...
    def write_config_file(self, **kwargs):
        section_name = kwargs.pop("section_name", self.config_class.config_section)
        config = "\n".join(
            ["[%s]" % (section_name,)] + ["%s = %s" % pair for pair in kwargs.items()]
        )
    ...
    def test_get_config_filename_precedence(self):
        ...
        self.config.default_config_filenames[:] = [default_filename1, default_filename2]
    ...
    def test_get_config_object(self):
        ...
        self.assertEqual(self.config.get_config_filename(), config_obj.filename)
    ...
    def test_data_directory_option(self):
        ...
        options = self.parser.parse_args(["--data-path", "/opt/hoojy/var/run"])[0]
    ...
    def test_load_not_found_accept_missing(self):
        ...
        with self.assertRaises(SystemExit) as cm:
            config.load(["--config", filename], accept_nonexistent_default_config=True)
        self.assertEqual(
            str(cm.exception), "error: config file %s can't be read" % filename
        )
    ...
    def test_load_cannot_read_default(self):
        ...
        with self.assertRaises(SystemExit) as cm:
            self.config.load([])
        self.assertEqual(
            str(cm.exception), "error: config file %s can't be read" % default
        )
    ...
    def test_load_cannot_read_many_defaults(self):
        ...
        with self.assertRaises(SystemExit) as cm:
            self.config.load([])
        self.assertEqual(str(cm.exception), "error: no config file could be read")
