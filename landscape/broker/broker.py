...
class BrokerDBusObject(Object):
    ...
    which gets broadcasted over the bus and eventually received by the
    other Landscape client processes.

    In order to be receive D-Bus signals from the broker the Landscape
    client processes have to register themselves as L{BrokerPlugin}s
    using the L{register_plugin} method. The L{BrokerPlugin.message}
    method of each plugin will be called on every C{"message"} evet
    generated by the L{TwistedReactor}.
    """
...
""" % (mtype,)
    ...
    def register_plugin(self, bus_name, object_path):
        ...
        Examples of plugins are L{MonitorDBusObject} and L{ManagerDBusObject}
        from the monitor and manager Landscape services respectively.
        """
